...
export default function UpdateCompanyForm({
  ...
  useEffect(
    () => {
      if (data) {
        ...
        if (data?.phoneNumber)
          setCompanyPhone({
            ...validatePhone((data?.countryCode || '+966') + '' + data?.phoneNumber, false),
            value: data?.phoneNumber,
          })
        setCompanyEmail({ ...validateEmail(data?.email || ''), value: data?.email || '' })

        form.setFieldValue('industry', data?.industry)
        form.setFieldValue('bankId', data?.bankInfo?.bankId)
        setArDescription({
        ...
        form.setFieldValue('regularLicensingDocUrl', data?.bankInfo?.regularLicensingDocUrl)
  ...
  const onFinish = async (values: any) => {
    if (
      phone.value &&
      phone.errorMsg === null &&
      companyPhone.errorMsg === null &&
      ownerEmail.value &&
      ownerEmail.errorMsg === null &&
      companyEmail.errorMsg === null
    ) {
      setLoading(true)
      try {
        ...
        let result = await companiesServiceInstance.update(values)

        message.success(
          t('TheCompnayHasBeenSuccessfullyUpdated', {
              ...
              {regularLicensingDocUrl && !isRegularLicensingDocUrlUploading ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <FileUploader
                    ...
                    handleChange={(e: any) => uploadImage(e, 'regularLicensingDocUrl')}
    ...
    },
    {
      ...
      children: (
        <Row gutter={40}>
          ...
          <Col lg={drawer ? 12 : 8} xs={24}>
            <Form.Item rules={[new Rules().getMandatoryRule()]} label={t('City')} name="cityId">
              <Select
                disabled={completeInfo}
        ...
        <Row gutter={40}>
          ...
          <Col xs={24} lg={drawer ? 12 : 8}>
            <Form.Item
              ...
              rules={[new Rules().getMandatoryRule()]}
            >
              <Input onChange={onOwnerEmailChange} autoComplete="off" value={ownerEmail.value} />
            </Form.Item>
          </Col>
          <Col xs={24} lg={drawer ? 12 : 8}>
  ...
  return (
    <Form
      ...
      className={`flex flex-col gap-3 w-full ${drawer ? 'h-full' : 'h-[450px] overflow-y-auto'} overflow-x-hidden  text-white mx-auto min-w-full`}
      colon={false}
      layout="vertical"
    >
      <Collapse
